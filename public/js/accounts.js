...
window.bridge.updateAccounts((event, data) => {
    ...
    const mainUserBoxList = document.querySelectorAll(".user.main");

    mainUserBoxList.forEach((mainUserBox) => {
        mainUserBox.setAttribute("onlick", `changeUser('${data.current.id}')`);
        mainUserBox.querySelector("img").src = `https://visage.surgeplay.com/face/512/${data.current.id}`;
        mainUserBox.dataset.id = data.current.id;
        mainUserBox.querySelector(".username").innerHTML = data.current.name;
        mainUserBox.querySelector(".faceBox").setAttribute("onclick", `removeUser('${data.current.id}')`);
    });
